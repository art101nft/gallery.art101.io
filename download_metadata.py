            ...
            with open(token_metadata, "r") as f:
                ...
                if len(parts) > 1:
                    ...
                    if not fullsize_path.exists():
                        ...
                        res = subprocess.run(['convert', image_path, fullsize_path], capture_output=False)
                    thumbnail_path = Path(image_folder, parts[1] + ".thumbnail.png")
                    if not thumbnail_path.exists():
                        print(f"Creating {c.title} {token_id} thumbnail")
                        res = subprocess.run(['convert', '-resize', '20%', image_path, thumbnail_path], capture_output=False)
                else:
